...
const SO_SHA = "49c3991219f85a4f3b2a856fb1207eee2897ffe2d7219c195f45141441cac497";
const closedEnv = {
...
assert.equal(provenance, "size=214088\nsha256=" + SO_SHA + "\nentry=0x2D068\n");
assert.deepEqual(helpers.validateClosedLoopProvenanceManifest(provenance.replace("0x2D068", "0x25950")), ["manifest line 3 mismatch"]);
assert.deepEqual(
  helpers.validateClosedLoopProvenanceManifest(provenance.replace(SO_SHA, "9".repeat(64))),
  ["manifest line 2 mismatch"]
);
assert.deepEqual(helpers.validateClosedLoopProvenanceManifest(provenance.replace("214088", "177936")), ["manifest line 1 mismatch"]);
...
assert.throws(() => helpers.closedLoopProvenanceUploadCommand(closedEnv, provenance.replace("0x2D068", "0x25950"), manifestSha));
assert.throws(() => helpers.closedLoopProvenanceUploadCommand(closedEnv, provenance.replace(SO_SHA, "9".repeat(64)), manifestSha));
assert.throws(() => helpers.closedLoopProvenanceUploadCommand(closedEnv, provenance.replace("214088", "177936"), manifestSha));
...
assert.equal(artifact.length, 214088);
assert.equal(createHash("sha256").update(artifact).digest("hex"), SO_SHA);
assert.equal(artifact.readBigUInt64LE(24).toString(16).toUpperCase(), "2D068");
